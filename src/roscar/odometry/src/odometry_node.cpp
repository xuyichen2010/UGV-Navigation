...
OdometryNode::OdometryNode(
        ...
        ros::NodeHandle private_nh) :
			...
			private_nh_.param("parent_frame_name", parent_frame_name_, std::string("odom"));
			private_nh_.param("child_frame_name", child_frame_name_, std::string("base_link"));
...
void OdometryNode::gpsCallback(const phidget_gps::Data::ConstPtr& msg) {
    ...
    //std::cout << "careAboutGPS_ : " << careAboutGPS_  << std::endl;
    ...
    if( careAboutGPS_ && (haveFix==1) ){
        ...
        double easting = p_utm.easting- initialX;
        double northing = p_utm.northing- initialY;
        double altitude  = p_utm.altitude;
       std::cout << "GPS corrdinate East: " << easting << "NOrth: "<< northing<< "altitude: " << altitude << std::endl;
...
initialY = northing;
initialSet = true;
std::cerr << "initialX: " << initialX << ", initialY: " << initialY << std::endl; 

}
            
           std::cout << "Have our first fix!" << std::endl;
            position(0) = easting - initialX;
            position(1) = northing - initialY;
            position(2) = heading;
            initialFix = true;
std::cerr << "position(0): " << position(0) << ", position(1): " << position(1) << std::endl; 
        }
    }
}
...
nav_msgs::Odometry OdometryNode::timeUpdatePhase(){  
    ...
    boost::mutex::scoped_lock lock(ekf_lock_);    
    {
        ...
        //xPrime = ( velocity * dt * ( cos (theta) ) );
        //yPrime = ( velocity * dt * ( sin (theta) ) );
    }
    {    
        boost::mutex::scoped_lock lock(imu_lock_);        
...
//std::cerr << "xCurr: " << xCurr << ", yCurr: " << yCurr << std::endl; 
//std::cerr << "xPrime: " << xPrime << ", yPrime: " << yPrime << std::endl; 
    msg.pose.pose.position.z = 0.0;

    //set up the twist elements of the message
    msg.twist.twist.linear.x = velocity * cos(theta);
    msg.twist.twist.linear.y = velocity * sin(theta);
    msg.twist.twist.angular.z = angularVelocity;

    //turn our yaw into a quaternion
   /* tf::Quaternion q;

    q.setRPY(0,0,theta); //roll = 0, pitch = 0, yaw = thetaPrime . thetaPrime
    ...
    //ROS_INFO_STREAM("Position: x: " << msg.pose.pose.position.x << ", y: " << msg.pose.pose.position.y<< ", theta: " << theta<<" angular theta:" << angularVelocity);
    return msg;
}
