...
fig, ax = plt.subplots()
#create data
#target_coords = [(x,np.log(x)) for x in np.arange(3,6,0.5) if x >= 1]
target_coords = [(0,x) for x in np.arange(1,10,0.5)[20:0:-1] ]

#x_range = np.arange(-30,30,1)
#y_range = [np.log(y) for y in np.arange(-30,30,1)]
#print(y_range)
#line, = ax.plot(x_range, y_range)	
#ax.set_xlim(-30,30)
#ax.set_ylim(-30,30)
ax.autoscale(enable=True, axis='both', tight=False)
scat = ax.scatter(target_coords[0][0],target_coords[0][1] )
#scat = ax.scatter(target_coords[0][0]*1.5,target_coords[0][1]*1.5 )


ani = animation.FuncAnimation(fig, animate, target_coords, interval=100, repeat=False)
